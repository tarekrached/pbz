...
export function connect(host, { WebSocketImpl = WebSocket, idleMs = IDLE_CLOSE_MS } = {}) {
  ...
  const alive = () => {
    ...
    if (ws.readyState !== 1 /* OPEN */) {
      throw new Error(`websocket: connection to ${host}:81 is no longer open (readyState ${ws.readyState})${RECOVERY}`);
    }
  };
  ...
  const collectFrames = async (need, ms = 6000) => {
    ...
    if (!isDead) json({ sendUpdates: false }); // teardown is pointless once the socket is gone
    q.purgeBinary(5);
    return result;
  };
