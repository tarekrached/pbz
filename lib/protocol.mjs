...
export function connect(host, { WebSocketImpl = WebSocket, idleMs = IDLE_CLOSE_MS } = {}) {
  ...
  const die = (err, detailed = true) => {
    if (isDead && (deadIsDetailed || !detailed)) return;
    isDead = true;
    deadIsDetailed = detailed;
  ...
  const opened = new Promise((res, rej) => {
    ...
    ws.onerror = (e) => {
      if (isOpen) { die(midExchangeError(e), Boolean(e?.error?.message || e?.message)); return; }
      clearTimeout(timer);
  ...
  const alive = () => {
    ...
    if (ws.readyState !== 1 /* OPEN */) {
      ...
      throw new Error(`websocket: the connection to ${host}:81 is no longer open (readyState ${ws.readyState})${RECOVERY}`);
    }
  };
