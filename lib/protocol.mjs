...
export function connect(host, { WebSocketImpl = WebSocket, idleMs = IDLE_CLOSE_MS } = {}) {
  ...
  // path. `isDead` is assigned further down but only read at call time.
  const using = (fn) => (...args) => { if (!isDead) touch(); return fn(...args); };
  ...
  //
  // `isDead` is a SEPARATE flag rather than `deadReason` truthiness. Guarding on
  // the string would let an Error with an empty message leave the connection
  // un-dead: alive() would never throw, _getConn would keep handing the corpse
  // back, and a second die() would overwrite the first cause — which the queue's
  // factory closes over, so even already-parked waiters would get the wrong one.
  // Chunk 29 stored the Error object and was immune; narrowing to a string is
  // what introduced the hazard, and this flag is what closes it again.
  let isDead = false;
  let deadReason = null;
  const deadError = () => new Error(deadReason);
  const die = (err) => {
    if (isDead) return;
    isDead = true;
    deadReason = err.message || String(err);
  ...
  // Two checks, because `isDead` alone is not enough. A real socket flips
  ...
  const alive = () => {
    if (isDead) throw deadError();
    if (ws.readyState !== 1 /* OPEN */) {
      // die() rather than a bare throw: the socket is unusable, so anything
      // parked on it should fail NOW instead of running out its own timeout and
      // then reporting a timeout — which is the entire point of Chunk 29. It
      // also stops `dead()` reading null on a connection nobody can use.
      die(new Error(`websocket: the connection to ${host}:81 is no longer open${RECOVERY}`));
      throw deadError();
    }
  };
  ...
  const sendBytecode = (blob, type = 3) => { // chunked binary frames: [type][flag] + <=1280 bytes
    alive();
    touch();
    // Checked PER CHUNK, not once. The loop below is synchronous, so `die()`
    // cannot interleave — but readyState can already be closed from a close
    // frame processed earlier, and `send()` discards silently on a closed
    // socket. Without the per-chunk check a death partway through delivered
    // chunk 1 of N and still returned, which is precisely what callers read as
    // "every chunk is on the socket". save()'s putSourceCode is multi-chunk.
  ...
  const collectFrames = async (need, ms = 6000) => {
    ...
    // socket, so guarding only on `isDead` let this teardown throw a complete
  ...
  return {
    ...
    collectChunks: (type, opts = {}) => { if (!isDead) touch(); return q.collectChunks(type, { ...opts, onProgress: () => { if (!isDead) touch(); } }); },
    json, sendBytecode, collectFrames,
    /** A fresh Error describing why the connection died, or null while healthy. */
    dead: () => (isDead ? deadError() : null),
