...
export function connect(host, { WebSocketImpl = WebSocket, idleMs = IDLE_CLOSE_MS } = {}) {
  ...
  // path. `deadReason` is assigned further down but only read at call time.
  const using = (fn) => (...args) => { if (!deadReason) touch(); return fn(...args); };
  ...
  // The REASON, not one Error object. Every throw and every rejection gets its
  // own instance built from it: handing the same instance to every caller means
  // anything that annotates a caught error (pixelblaze.mjs's device-state note)
  // writes on an object other callers also hold, so a concurrent read can pick
  // up a save's annotation. Flagged as cosmetic in Chunk 29's review; Chunk 30
  // is the thing that made it consequential.
  let deadReason = null;
  const deadError = () => new Error(deadReason);
  const die = (err) => {
    if (deadReason) return;
    deadReason = err.message;
  ...
  const alive = () => { if (deadReason) throw deadError(); };
  ...
  const collectFrames = async (need, ms = 6000) => {
    ...
    if (!deadReason) json({ sendUpdates: false }); // teardown is pointless once the socket is gone
    q.purgeBinary(5);
    return result;
  };
  ...
  return {
    ...
    collectChunks: (type, opts = {}) => { if (!deadReason) touch(); return q.collectChunks(type, { ...opts, onProgress: () => { if (!deadReason) touch(); } }); },
    json, sendBytecode, collectFrames,
    /** The error the connection died of, or null while it is healthy. */
    dead: () => (deadReason ? deadError() : null),
