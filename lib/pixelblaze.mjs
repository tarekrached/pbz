...
import { parseFileList, parseBackup, buildBackup } from './backup.mjs';
...
export class Pixelblaze {
  ...
  async list() {
    ...
    while ((frame = await c.waitBinary(1500, 7))) buf = Buffer.concat([buf, frame.subarray(2)]);
  ...
  async getSources(id) {
    ...
    let buf = Buffer.alloc(0), frame;
    while ((frame = await c.waitBinary(1500, 6))) buf = Buffer.concat([buf, frame.subarray(2)]);
  ...
  async getPreviewImg(id) {
    ...
    let buf = Buffer.alloc(0), frame;
    while ((frame = await c.waitBinary(1500, 4))) buf = Buffer.concat([buf, frame.subarray(19)]);
    return buf;
  }
  ...
  async getPlaylist() {
    ...
    const msg = await c.waitText('{"playlist"');
  ...
  async saveBackup(file) {
    ...
    if (!outFile) {
      ...
      const name = (cfg.name || 'pixelblaze').replace(/[^\w.-]+/g, '_');
      outFile = `${name}-${new Date().toISOString().slice(0, 10)}.pbb`;
    }
  ...
  async ping() {
    ...
    const t0 = Date.now();
    c.json({ ping: true });
    const msg = await c.waitText('{"ack"');
