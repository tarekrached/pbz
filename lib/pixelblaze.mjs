...
//
// THE HONESTY RULE IS THE LINE ORDER. A `maybe-` state is assigned BEFORE its
// send (we put bytes on the wire and heard nothing back); a bare state is
// assigned AFTER the device's own ack. A reviewer can check that a claim is
// earned by looking at where the assignment sits, without reading the prose.
//
...
const DEVICE_LEFT = {
  'maybe-paused': () =>
    ...
    ' (`pbz info` reads fps 0). Re-run this command, or `pbz activate "<a saved pattern>"` to resume rendering',
  'running-unsaved': () =>
    'the compiled pattern IS rendering on the device now, but nothing reached flash, so it is absent from' +
    ' `pbz list` and a reboot restores whatever was active before',
  'maybe-saved': (name) =>
    'the pattern data was sent but never acknowledged. A partial transfer commits nothing, so if ' + name +
    ' appears in `pbz list` then the write did complete and only the acknowledgement was lost — delete it before retrying',
  'saved-maybe-inactive': (name) =>
    'it IS saved to the device (the write was acknowledged) and is rendering now, but the activation was not' +
    ` confirmed, so the device may revert to the previously active pattern on reboot. \`pbz activate ${name}\` finishes the job`,
};
...
function withDeviceState(e, state, { id, name } = {}) {
  if (!state || !DEVICE_LEFT[state] || e?.device) return e;
  e.device = id ? { state, id } : { state };
  // run() saves nothing, so it has neither a name nor an id. Only the two
  // save-side states interpolate a name, but keep the fallback sane rather
  // than letting a bare `undefined` reach a user-facing string.
  const label = name === undefined ? 'the pattern' : JSON.stringify(name);
  e.message += ` — ${DEVICE_LEFT[state](label)}.`;
  return e;
}
...
export class Pixelblaze {
  ...
  async run(source) {
    ...
    try {
      ...
      left = null; // rendering again, and that is this method's success state
      ...
      // Swallowed: this claim exists only to protect a FUTURE call on this
      // connection, and a dead connection has none — the next call reconnects.
      // Letting it throw would report failure for a pattern already running.
      await c.waitText('{"ack"', 1000, m).catch(() => null);
      await sleep(150);
      return { program, bytecode };
    } catch (e) {
      throw withDeviceState(e, left);
    }
  }
  ...
  async save(source, name, opts = {}) {
    ...
    // are assigned BEFORE their send, bare ones AFTER the device's ack — see
    // withDeviceState. Nothing above this line has touched the wall.
    let left = null;
    try {
      let m = c.mark();
      left = 'maybe-paused';
      ...
      left = 'running-unsaved';
      // two commands were sent, so two acks come back; claim the second or it is
      // left to be matched by whatever asks for an ack next (ping, notably).
      // Unlike run()'s identical claim this one is NOT swallowed: save() has
      // four steps left, so a dead connection here is a real failure.
      ...
      throw withDeviceState(e, left, { id, name });
    }
  }
