...
const DEVICE_LEFT = {
  'maybe-paused': () =>
    ...
    ' (`pbz info` reads fps 0). Re-run this command, or activate a saved pattern, to resume rendering',
  ...
  'running-unsaved': () =>
    'the compiled pattern IS live on the device (it acknowledged the resume), but nothing reached flash, so it' +
    ' is absent from `pbz list` and a reboot loses it',
  'maybe-saved': (name) =>
    ...
    ' already be in `pbz list` from an earlier save, so its presence there does not tell you this one landed',
  ...
  'saved-maybe-inactive': (name) =>
    ...
    ` this one is likely to come back as the default whether you wanted it or not. \`pbz activate ${name}\`` +
...
export class Pixelblaze {
  ...
  async run(source) {
    ...
    try {
      ...
      const resumed = Boolean(await c.waitText('{"ack"', 1000, m).catch(() => null));
      if (!resumed) {
        throw new Error('run: the device never acknowledged the resume, so the pattern may be loaded but not rendering');
      }
  ...
  async save(source, name, opts = {}) {
    ...
    let left = null;
    let resumed = false; // did the device confirm the pause was lifted?
    try {
      let m = c.mark();
      ...
      // Deliberately tolerant of null rather than fail-loud: if this firmware
      // ever sends one ack for the pair, a hard throw here would break every
      // save, whereas staying at `maybe-paused` merely under-claims.
      resumed = Boolean(await c.waitText('{"ack"', 1000, m));
      if (resumed) left = 'running-unsaved';
      await sleep(200);
      ...
      // So a lost ack is forgiven the moment the device draws something.
      resumed = true;
      ...
      for (let i = 0; i < expectedAcks; i++) {
        const frame = await expectAck(c, m, 'save: putSourceCode');
        if (frame.includes('saveProgramSourceFile')) { left = 'saved-maybe-inactive'; break; }
      }
      // Left at `maybe-saved` if the completion marker never appeared: a
      // firmware that does not send it is under-claimed rather than lied about.
      await sleep(200);
      m = c.mark();
