...
//
// Concurrent reads are only safe when they claim DIFFERENT frames. getInfo()'s
// own Promise.all relies on that (getConfig + getStatus + getPeers are
// disjoint), but getVars and getState both claim `{"vars"`, and getConfig and
// getState both claim `{"name"` and `{"activeProgram"`. The same overlap is
// what makes some reads unsafe alongside a WRITE: getConfig() and getState() each
// claim an `{"activeProgram"` frame, which save() and activate() need; ping()
// claims an `{"ack"`, which save()'s chunk accounting, setControls(),
// setConfig() and delete() all need; and samplePreview() purges type-5 frames
// and stops the preview stream a save() is mid-collection on. The reads that
// claim neither an ack nor an activeProgram frame — getStatus(), list(),
// getPeers(), getVars(), getSources(), getPreviewImg(), getMap() — are safe
// alongside anything. See PBZ-PLAN.md's item 15.
...
// protocol.mjs's `alive()`, which refuses on any socket that is not OPEN —
// without it the send would discard silently and return as if it had worked,
// and this whole rule would rest on nothing.
//
// Every claim in these messages was verified against real hardware; several are
// counter-intuitive and the chunk entry records what was measured.
const DEVICE_LEFT = {
  'maybe-paused': (_name, { host }) =>
    ...
    ` (\`pbz info --host=${shellQuote(host)}\` reads fps 0). Re-run this command, or activate a saved pattern,` +
    ' to resume rendering',
  // Two deliberate choices, both easy to "tidy" back into falsehoods. PAST
  // tense, because the transport error this appends to usually reads "it may
  // have rebooted" and a reboot falsifies a present-tense "IS live" in the same
  // breath as suggesting it. And it does NOT name its evidence: the state is
  // reached either by the resume ack or by the preview frames when that ack was
  // lost, so "(it acknowledged the resume)" would be false on the frames path.
  'running-unsaved': (_name, { host }) =>
    ...
    ` is absent from \`pbz list --host=${shellQuote(host)}\` and does not survive a reboot — including one that may have just ended this connection`,
  'maybe-saved': (name, { host }) =>
    ...
    ` already appear in \`pbz list --host=${shellQuote(host)}\` from an earlier save, so its presence there does not tell you` +
    ' this one landed',
  ...
  // The command must carry --host (without it, advice printed during a
  // `--host=<spare>` save would change the actual wall) and must target the id
  // (`_resolveTarget` takes the first NAME match, and the spare really does
  // have two patterns called "Newfire"). Both interpolations are shell-quoted
  // because `import()` passes the `.epe`'s OWN id through as `opts.id`, so it
  // is not necessarily `stableId()` output.
  'saved-maybe-inactive': (name, { host, id }) =>
    ...
    ` this one is likely to come back as the default whether you wanted it or not. \`pbz activate` +
    ` --host=${shellQuote(host)} ${shellQuote(id)}\` settles it either way (the id, not ${name}: two saved` +
    ' patterns really can share a name)',
};

// A "paused AND saved" combination cannot occur, so do not add one: every
// save-side state is downstream of the preview frames, and frames prove the
// renderer is running, so an unconfirmed resume can only be `maybe-paused`.
...
function withDeviceState(e, state, { id, name, host } = {}) {
  ...
  // ONE predicate, expressed once. Frozen, sealed, and read-only-`device` all
  // fail the same way — the assignment throws — so an `isExtensible` check in
  // the guard above would be a second spelling of this catch, and whichever ran
  // second would be unreachable. (It was: for two rounds the guard clause and
  // this catch traded places, and deleting either changed nothing observable.)
  // Handing back an unannotated error is the honest outcome: it reads as
  // "nothing was sent", which is wrong here, but inventing a TypeError about
  // this helper in place of the real failure is worse.
  try {
  ...
  // Only the append is guarded, and only for one case the checks above cannot
  // catch: an extensible object whose `message` is a getter with no setter
  // (DOMException), where `+=` throws. Losing the prose is survivable; losing
  // the error it describes is not.
  try {
    e.message += ` — ${DEVICE_LEFT[state](label, { host, id })}.`;
  } catch { /* unwritable message: the structured `device` above still carries it */ }
  return e;
}
...
// How long run() waits for the device's own status frame to confirm it is
// actually rendering. The device broadcasts one roughly every second and run()
// already spends ~2s getting here, so one has usually arrived already and this
// costs nothing; the budget only bites on a device that has gone quiet, which
// is the case worth waiting for.
const STATUS_CONFIRM_MS = 1200;
...
export class Pixelblaze {
  ...
  async run(source) {
    ...
    try {
      ...
      // Claim the pair's second ack so it cannot answer a later wait. Its
      // ARRIVAL proves nothing: acks carry no request id, and the multi-chunk
      // bytecode send above is acked frame-by-frame, so a straggler is
      // indistinguishable from the resume's own. A fuzz run used exactly that
      // to make this method RETURN SUCCESS over a frozen wall — `pbz run`
      // printing "ok (live, not saved)" at a dark room, which is the false
      // success this whole chunk exists to remove.
      const resumeAck = await c.waitText('{"ack"', 1000, m);
      // So confirm with something that cannot be mistaken for anything else:
      // the device's own unsolicited status frame, which it broadcasts about
      // once a second. `fps > 0` is rendering; `fps 0` is the frozen wall, and
      // is positive evidence rather than an absence.
      const status = await c.waitText('{"fps"', STATUS_CONFIRM_MS, m);
      const fps = status ? JSON.parse(status).fps : null;
      if (fps === 0) {
        throw new Error('run: the device reports fps 0 after the resume, so the pattern is loaded but not rendering');
      }
      if (!resumeAck && fps === null) {
        // No ack and no status frame: nothing says this worked.
        throw new Error('run: the device neither acknowledged the resume nor reported rendering, so the pattern may be loaded but not running');
      }
      left = null; // rendering, or at least nothing says otherwise
      await sleep(150);
      return { program, bytecode };
    } catch (e) {
  ...
  async save(source, name, opts = {}) {
    ...
    try {
      ...
      // Claim the pair's second ack so it cannot answer a LATER wait — but do
      // NOT read it as evidence of anything. Acks carry no request id, and the
      // multi-chunk sendBytecode above is itself acked frame-by-frame, so on a
      // board slow enough to matter those stragglers land after this mark and
      // get counted here. A fuzz run over the whole failure space caught it:
      // one unmatched ack bought `running-unsaved` outright, and the zero-frame
      // error below then answered its own "Is the pattern rendering?" with "it
      // was confirmed rendering" over a frozen wall — item 8's original bug,
      // through a different door. The preview frames are unambiguous and arrive
      // moments later, so this method waits for those and never promotes on an
      // ack it cannot identify.
      ...
      if (!frames.length) {
        // Zero frames has two opposite causes, both measured live, and the
        // device itself can tell them apart. Either the wall really is frozen
        // (a lost resume), or it is rendering fine and a CONCURRENT READER took
        // the stream — `samplePreview()`'s teardown sends `sendUpdates:false`
        // and purges type-5 frames, which was reproduced against real hardware
        // stopping a save's collection mid-flight. Ask for the device's own
        // status frame: fps is unambiguous where the frame count is not.
        const status = await c.waitText('{"fps"', STATUS_CONFIRM_MS, m);
        const fps = status ? JSON.parse(status).fps : null;
        if (fps > 0) left = 'running-unsaved'; // rendering; the stream just was not ours
        throw new Error(
          'save: device sent no preview frames, so the thumbnail would be empty — the web UI rejects patterns saved that way.' +
          (fps > 0
            ? ' The device reports it IS rendering, so something else on this connection likely took the preview stream — a concurrent read (`samplePreview`, `pbz power`) will do it. Retry with nothing else talking to the device.'
            : ' Is the pattern rendering?')
        );
      }
  ...
  async defrag(file, opts = {}) {
    ...
    for (let i = 0; i < deletable.length; i++) {
      ...
      if (!res.ok) {
        throw new Error(
          ...
          `A verified backup is at ${backupFile}; recover with: pbz restore ${shellQuote(backupFile)} --host=${shellQuote(this.host)} --yes`
        );
      }
    }
    ...
    try {
      ...
      throw new Error(
        ...
        `a verified backup is at ${backupFile}; retry with: pbz restore ${shellQuote(backupFile)} --host=${shellQuote(this.host)} --yes`
      );
    }
    ...
    try {
      ...
      throw new Error(
        ...
        `the .pbb at ${backupFile} matches what was sent; check manually: pbz list --host=${shellQuote(this.host)}`
      );
    }
