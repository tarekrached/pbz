...
// send RETURNS — the sends are synchronous and throw on a dead connection
// before a byte moves, so returning is what proves the bytes reached the
// socket — and a bare state AFTER the device's own ack. A reviewer can check
// that a claim is earned by looking at where the assignment sits, without
// reading the prose.
//
...
const DEVICE_LEFT = {
  ...
  // Past tense on purpose, and it took reading the real CLI output to see why:
  // the transport error this note appends to usually reads "it may have
  // rebooted", and a reboot falsifies a present-tense "IS live" in the same
  // breath as suggesting it. What is actually known is that the device was
  // rendering when it last spoke to us.
  'running-unsaved': () =>
    'the compiled pattern WAS live on the device (it acknowledged the resume), but nothing reached flash, so it' +
    ' is absent from `pbz list` and does not survive a reboot — including one that may have just ended this connection',
  'maybe-saved': (name) =>
