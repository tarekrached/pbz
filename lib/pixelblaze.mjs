...
function withDeviceState(e, state, { id, name, host } = {}) {
  ...
  // `isExtensible` matters as much as the type checks: a frozen or sealed Error
  // cannot take `device` at all, and silently returning it WITHOUT the property
  // would read as "nothing was sent" — a documented contract, and a lie at a
  // moment when the pattern is live on the wall. Better to annotate nothing and
  // say nothing than to say the opposite of the truth.
  if (typeof e !== 'object' || e === null || e.device || typeof e.message !== 'string' || !Object.isExtensible(e)) return e;
  // Outside the try ON PURPOSE. Inside it, a thrown string took the same path
  // whether or not the guard above existed, which made the guard — and the test
  // for it — unobservable.
  // Its own try: `isExtensible` does not cover a non-writable or getter-only
  // `device` property, and assigning to one of those throws — replacing the
  // real failure with a TypeError about this helper, the exact outcome the
  ...
  async defrag(file, opts = {}) {
    ...
    for (let i = 0; i < deletable.length; i++) {
      ...
      if (!res.ok) {
        throw new Error(
          ...
          `A verified backup is at ${backupFile}; recover with: pbz restore ${backupFile} --host=${shellQuote(this.host)} --yes`
        );
      }
    }
    ...
    try {
      ...
      throw new Error(
        ...
        `a verified backup is at ${backupFile}; retry with: pbz restore ${backupFile} --host=${shellQuote(this.host)} --yes`
      );
    }
