...
// relies on it), but not all of them are safe alongside a WRITE, because some
// wait on frames a write is also waiting for: getConfig() and getState() each
...
const DEVICE_LEFT = {
  ...
  // the id is exact — verified live, this spare has two patterns both named
  // "Newfire", and activating by name silently takes the first.
  //
  // BOTH interpolations are shell-quoted, including the id: `import()` passes
  // `epe.id` from the FILE straight through as `opts.id`, so it is not
  // necessarily `stableId()` output. An earlier comment here claimed an id
  // "cannot be eaten by argv as a flag" — true for a generated id, false for a
  // file-supplied one, and a `.epe` carrying a backtick would have executed on
  // paste.
...
function withDeviceState(e, state, { id, name, host } = {}) {
  ...
  // Only the message append is guarded: `message` can be a getter-only accessor
  // (DOMException, and some subclasses) with no setter, where `+=` throws.
  // Losing the prose is survivable; losing the error it describes is not.
  try {
  ...
  async defrag(file, opts = {}) {
    ...
    for (let i = 0; i < deletable.length; i++) {
      ...
      if (!res.ok) {
        throw new Error(
          ...
          `A verified backup is at ${backupFile}; recover with: pbz restore ${backupFile} --host=${this.host} --yes`
        );
      }
    }
    ...
    try {
      ...
      throw new Error(
        ...
        `a verified backup is at ${backupFile}; retry with: pbz restore ${backupFile} --host=${this.host} --yes`
      );
    }
