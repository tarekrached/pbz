...
export class Pixelblaze {
  ...
  // Status-popup contents: firmware/hardware from getConfig, live stats from
  // the unsolicited status frame every connection receives ~1/s (fps, mem,
  // uptime, storage — no request needed, confirmed live), and peers from
  // getPeers. `exp` is a bitmask (verified in the web UI's own status-popup
  // handler: bit1 = Sensor Board 1.0, bit2 = 6-axis). Group role mirrors the
  // web UI's own logic: Leader if peers report following us, Follower if we
  // have a leaderId, else Solo (this rig is always Solo — CLAUDE.md invariant
  // #5, single controller by design).
  async getInfo() {
    const c = connect(this.host); await c.opened;
    c.json({ getConfig: true });
    c.json({ getPeers: 1 });
    const [cfgMsg, statusMsg, peersMsg] = await Promise.all([
      c.waitText('{"name"'),
      c.waitText('{"fps"'),
      c.waitText('{"peers"'),
    ]);
    c.close();
    if (!cfgMsg) throw new Error('getInfo: no config response from device');
    const cfg = JSON.parse(cfgMsg);
    const status = statusMsg ? JSON.parse(statusMsg) : {};
    const peers = peersMsg ? JSON.parse(peersMsg).peers : [];
    const followers = peers.filter(p => p.isFollowing);
    return {
      version: cfg.ver, boardType: cfg.boardType, chipId: cfg.chipId,
      fps: status.fps, mem: status.mem, uptimeMs: status.uptime,
      storageUsed: status.storageUsed, storageSize: status.storageSize,
      expansion: { sensorBoard: !!(cfg.exp & 1), sixAxis: !!(cfg.exp & 2) },
      nodeId: cfg.nodeId, leaderId: cfg.leaderId,
