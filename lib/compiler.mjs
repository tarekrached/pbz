...
// Build normalizeMap(rawCoords, fit) -> {pixelMap, dimensions} using the web
// UI's own normalizeMap (min/max per dimension, "Fill"/"Contain" fit blend,
// round to 0..65535) — the device can't run this itself (same reason it
// can't compile), so setMap/getMap({coords}) must, exactly like the pattern
// compiler. `$` is stubbed (only `.val()` on "#mapperFit" is real, wired to
// the `fit` arg) since the DOM doesn't exist headless; `mapperSourceLoaded`
// is force-reset false each call so the extracted code's own `sendMap()`
// side effect (fired only when that flag is already true) never triggers —
// pbz sends the frame itself, from the caller, using the return value.
export function makeNormalizeMap(html) {
  const start = html.indexOf('function normalizeMap(');
  if (start < 0) throw new Error('normalizeMap not found in web UI');
  const braceStart = html.indexOf('{', start);
  const end = matchBraces(html, braceStart);
  const src = html.slice(start, end + 1);
  const chain = { removeClass() { return chain; }, addClass() { return chain; }, find() { return chain; }, text() { return chain; } };
  const ctx = vm.createContext({
    pixelMap: undefined, pixelMapDimensions: 0, pixelMapSmallestRatio: 1,
    mapperSourceLoaded: false, __fit: 0,
    $: (sel) => (sel === '#mapperFit' ? { val: () => String(ctx.__fit) } : chain),
  });
  vm.runInContext(
    src + ';\nfunction __normalize(raw, fit) { __fit = fit; mapperSourceLoaded = false; normalizeMap(raw); return { pixelMap: pixelMap, dimensions: pixelMapDimensions }; }',
    ctx, { filename: 'pb-normalizeMap.js' });
  return (raw, fit = 0) => {
    ctx.__raw = raw;
    return vm.runInContext('__normalize(__raw, ' + fit + ')', ctx);
  };
}
