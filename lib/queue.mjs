...
export function makeQueues({ maxQueued = MAX_QUEUED, now = () => Date.now(), pollMs = 10 } = {}) {
  ...
  return {
    ...
    /**
     * The transport died: reject every parked waiter now, and every later one
     * on arrival, each with its OWN Error from `makeErr`. Idempotent — the first cause wins, because the
     ...
     */
    fail: (makeErr) => {
      // A caller passing an Error instead of a factory used to fail silently
      // when nothing was parked, and when something WAS parked the TypeError
      // escaped fail() -> escaped protocol.mjs's die() -> so its ws.close()
      // never ran and the dead socket got reused. Fail obviously instead.
      if (typeof makeErr !== 'function') throw new TypeError('queue.fail expects a factory function, not an Error');
      if (failure) return;
      failure = makeErr;
