...
export function makeQueues({ maxQueued = MAX_QUEUED, now = () => Date.now(), pollMs = 10 } = {}) {
  ...
  const waitEntry = (queue, match, ms, after) => new Promise((res, rej) => {
    if (failure) { rej(failure); return; }
    const t0 = now();
    const abort = (err) => { clearInterval(iv); pending.delete(abort); rej(err); };
    const settle = (v) => { clearInterval(iv); pending.delete(abort); res(v); };
    const tick = () => {
      const idx = queue.findIndex(e => e.seq > after && match(e.value));
      if (idx >= 0) { settle(queue.splice(idx, 1)[0]); return; }
      if (now() - t0 > ms) { settle(null); }
    };
  ...
  return {
    ...
    fail: (err) => {
      if (failure) return;
      failure = err;
      for (const abort of [...pending]) abort(err);
      pending.clear();
    },

    /** The failure `fail()` recorded, or null while the transport is healthy. */
    failure: () => failure,
     ...
     */
    collectChunks: async (type, { ms = 3000, after = -1, headerBytes = 2 } = {}) => {
      const parts = [];
      let since = after;
      for (;;) {
